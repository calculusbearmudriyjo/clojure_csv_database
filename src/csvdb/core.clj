...
(defn key-value-pairs [tbl-keys tbl-record]
  (remove nil? (flatten (map-indexed (fn [ind itm] (when-not (nil? (get tbl-record ind)) (vector itm (get tbl-record ind)))) tbl-keys))))
...
(defn data-table [tbl]
  (let [key (table-keys tbl) data (next tbl)] (map #(data-record key %) data)))
...
;; => ({:surname "Sidorov", :year 1996, :id 3} {:surname "Petrov", :year 1997, :id1 2} {:surname "Ivanov", :year 1998, :id 1})
;; Hint: if-not, sort-by
...
(defn join* [data1 column1 data2 column2]
  ...
  (into [] (concat (map (fn [red-el] (merge (first (filter (fn [el] (= (column2 el) (column1 red-el))) data2)) red-el)) data1)))
 )
